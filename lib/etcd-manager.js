...
var singleton = function singleton(){
    ...
    * @function
    */
    this._init = function (key, alias, host, port, _namespace) {
        ...
        var service = host + ':' + port + _namespace;
        var promise = this._serviceManager.registerService(key, service);
    ...
    *
    * @param key         The Key to be found in the Etdc Manager
    * @param alias       Friendly name to the service, only useful for logging
    * @param host        The host address
    * @param port        The service port
    * @param _namespace  The url namespace
    * @param _cb         Callback function which will receive the service URL
    */
    this.register = function (key, alias, host, port, _namespace) {
        ...
        promise.then(
            function onResolve() {
                ...
                console.log('[FATAL] ETCD not reached.: '.red +
                    JSON.stringify(err));
                throw err;
            }
        );
...
singleton.getInstance = function(){
    if(this.instance === null){
        ...
        try{
            ...
            etcd._init(cfg.key, cfg.alias, cfg.host, cfg.port, cfg.namespace);
            if(cfg.dependencies){
                cfg.dependencies.forEach(function(host) {
